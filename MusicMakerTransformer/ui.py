...
def train_progress():
    """
    Live training bar: step count, loss, rate, ETA.

    The raw step number is the thing you actually watch, so it is printed
    explicitly -- a percentage alone is useless on a 6-day run where step 2
    and step 200 both render as "0%".
    """
    if not _RICH:
        return None
    return Progress(
        ...
        # step N/total, before anything else
        TextColumn("[bold]{task.completed:>6.0f}[/][dim]/{task.total:.0f}[/]"),
